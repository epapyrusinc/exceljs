...
class WorkbookReader extends EventEmitter {
  ...
  async *parse(input, options) {
        ...
        default:
          if (entry.path.match(/xl\/worksheets\/sheet\d+[.]xml/)) {
            ...
            if (this.sharedStrings && this.workbookRels) {
              ...
              // Buffer worksheet data using PassThrough to avoid blocking the iterator
              // Note: We must NOT do async operations (like tmp.file) here as it breaks
              // the for-await iteration over the zip entries
              const buffer = new PassThrough();
              entry.pipe(buffer);
              waitingWorkSheets.push({sheetNo, buffer});
              // Wait for entry to be fully consumed before continuing to next entry
              await new Promise(resolve => entry.on('end', resolve));
            }
    ...
    for (const {sheetNo, buffer} of waitingWorkSheets) {
      yield* this._parseWorksheet(buffer, sheetNo);
    }
  }
