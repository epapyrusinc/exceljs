...
const ThemeXform = require('./xform/theme/theme-xform');
...
class XLSX {
  ...
  reconcile(model, options) {
    ...
    if (model.styles && model.styles.model) {
      model.styles = model.styles.model;
    }
  ...
  async _processThemeEntry(entry, model, name) {
    ...
    // preserve raw theme XML for round-tripping on write
    model.themesXml[name] = xml;
    const xform = new ThemeXform();
    const stream = new PassThrough();
    stream.end(xml);
    model.themes[name] = await xform.parse(parseSax(stream));
  }
  ...
  async addThemes(zip, model) {
    // Themes are round-tripped via raw XML (not the parsed CT_OfficeStyleSheet model).
    // When loading, we store parsed themes in `model.themes` AND raw XML in `model.themesXml`.
    // When writing, prefer the raw XML map to keep fidelity and ensure JSZip gets valid data.
    let themes = model.themesXml || model.themes;
    if (!themes || !Object.keys(themes).length) {
      themes = {theme1: theme1Xml};
    }
    Object.keys(themes).forEach(name => {
      let xml = themes[name];
      // If a consumer passed parsed theme models here, fall back to default theme XML.
      if (typeof xml !== 'string' && !(typeof Buffer !== 'undefined' && Buffer.isBuffer(xml))) {
        xml = theme1Xml;
      }
